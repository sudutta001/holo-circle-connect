          ...
          <section className="min-w-0 bg-paper/70">
            <header className="flex flex-wrap items-center justify-between gap-3 border-b-2 border-ink px-4 py-3 sm:px-6">
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="avatar-ink flex size-9 items-center justify-center rounded-full bg-sky font-display text-lg">{(displayProfile.display_name[0] ?? "G").toUpperCase()}</div>
              </div>
            </header>
...
function ProfileCard({ profile, tone, isFollowing, isConnected, onFollow, onConnect, onRoom }: { profile: Profile; tone: string; isFollowing: boolean; isConnected: boolean; onFollow: () => void; onConnect: () => void; onRoom: () => void }) {
  return <article className="noisy-hover sketch-border bg-panel p-4">
    <div className="flex items-start justify-between gap-3"><div className={`avatar-ink ${tone} flex size-14 shrink-0 items-center justify-center rounded-full font-display text-3xl`}>{profile.display_name[0]}</div><button className="p-1 hover:text-signal" title="More profile options" aria-label={`More options for ${profile.display_name}`}><Settings2 className="size-4" /></button></div>
